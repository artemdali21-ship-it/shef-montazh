...
/**
 * Hook to access Telegram WebApp context
 * Retries up to 10 times if user data is not yet available
 */
export const useTelegram = (retryCount: number = 0): any => {
  ...
  if (!tg.initDataUnsafe?.user) {
    ...
    if (retryCount < 10) {
      console.log(`[useTelegram] initDataUnsafe.user not yet available, retry ${retryCount + 1}/10`);
      // Return partial object that allows checks
      return {
        user: null,
        isReady: false,
        close: () => tg.close(),
      ...
      console.warn('[useTelegram] Failed to get user after 10 retries - Telegram may not be initialized');
    }
  }

  // Try to expand and set colors
  try {
    tg.expand();
    tg.setHeaderColor('#8B8B8B');
    tg.setBackgroundColor('#8B8B8B');
  } catch (error) {
    console.log('[useTelegram] Minor error setting styles:', error);
    // This is not critical, continue anyway
  }

  return {
    user: tg.initDataUnsafe?.user || null,
    isReady: !!tg.initDataUnsafe?.user,
    close: () => tg.close(),
    haptic: (style: string) => tg.HapticFeedback?.impactOccurred(style)
  };
};
