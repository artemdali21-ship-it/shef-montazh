...
function LoginPageContent() {
  ...
  useEffect(() => {
    let isMounted = true
    let retryCount = 0
    const maxRetries = 20 // Retry for up to 10 seconds (500ms * 20)

    const attemptLoad = () => {
      const freshTg = useTelegram()
      
      if (!isMounted) return

      if (freshTg?.isReady && freshTg?.user?.id) {
        console.log('[LoginPage] Telegram ready, loading roles...')
        loadUserRoles()
      } else if (retryCount < maxRetries) {
        retryCount++
        console.log(`[LoginPage] Telegram not ready yet, retry ${retryCount}/${maxRetries}`)
        setTimeout(attemptLoad, 500)
      } else {
        console.warn('[LoginPage] Telegram not available after retries')
  ...
  const loadUserRoles = async () => {
    try {
      if (!tg?.user?.id) {
        console.log('[LoginPage] Telegram user not ready')
        setLoading(false)
        return
      }
  ...
  const handleSelectRole = async (role: UserRole) => {
    ...
    try {
      ...
      console.log('[LoginPage] Signing in with telegramId:', tg.user.id, 'newRole:', role)
